      ...
      <div className="header-actions">
        ...
        {displayUser
          ? <div className="signed-header-actions">
              ...
              <button className={`profile-trigger${displayUser.blog?.profileImageUrl ? ' has-photo' : ''}`} style={displayUser.blog?.profileImageUrl ? { backgroundImage: `url(${displayUser.blog.profileImageUrl})` } : undefined} aria-label="프로필 메뉴" aria-expanded={profileOpen} onClick={() => setProfileOpen(!profileOpen)}>{displayUser.blog?.profileImageUrl ? '' : displayUser.nickname.slice(0, 1).toUpperCase()}</button>
              {profileOpen && <>
