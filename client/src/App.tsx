...
type User = { id: number; email: string; nickname: string; interests?: string[]; blog?: { id: number; name: string; slug: string; profileImageUrl?: string | null; subscriberCount?: number } | null }
...
type BlogCategory = { id: number; name: string; position: number; isDefault?: boolean; activePostCount: number; trashPostCount: number }
...
type Comment = { id: number; postId: number; parentId?: number | null; body: string; author: { id: number; nickname: string }; anonymous: boolean; deleted: boolean; createdAt: string; updatedAt: string }
      ...
      <div className="header-actions">
        ...
        {displayUser
          ? <div className="signed-header-actions">
              ...
              <button className={`profile-trigger${displayUser.blog?.profileImageUrl ? ' has-photo' : ''}`} style={displayUser.blog?.profileImageUrl ? { backgroundImage: `url(${displayUser.blog.profileImageUrl})` } : undefined} aria-label="프로필 메뉴" aria-expanded={profileOpen} onClick={() => { setProfileOpen(!profileOpen); setNotificationOpen(false) }}>{displayUser.blog?.profileImageUrl ? '' : displayUser.nickname.slice(0, 1).toUpperCase()}</button>
              {profileOpen && <>
                <button className="profile-menu-scrim" aria-label="프로필 메뉴 닫기" onClick={() => setProfileOpen(false)} />
                <div className="profile-popover" role="menu">
                  <div className="profile-summary"><span className={`profile-avatar${displayUser.blog?.profileImageUrl ? ' has-photo' : ''}`} style={displayUser.blog?.profileImageUrl ? { backgroundImage: `url(${displayUser.blog.profileImageUrl})` } : undefined}>{displayUser.blog?.profileImageUrl ? '' : displayUser.nickname.slice(0, 1).toUpperCase()}</span><div><strong>{displayUser.nickname}</strong><span>{displayUser.email}</span><button disabled={authPending} onClick={() => navigate('/blog/me/manage')}>계정관리</button></div></div>
...
function AccountPanel({ user, go }: { user: User; go: (to: string) => void }) {
  ...
  return <section className="account-dashboard">
    <div className="account-dashboard-head"><span className={`account-dashboard-avatar${user.blog?.profileImageUrl ? ' has-photo' : ''}`} style={user.blog?.profileImageUrl ? { backgroundImage: `url(${user.blog.profileImageUrl})` } : undefined}>{user.blog?.profileImageUrl ? '' : user.nickname.slice(0, 1).toUpperCase()}</span><div><strong>{user.nickname}</strong><span>구독자 <b>{user.blog?.subscriberCount ?? 0}명</b></span></div><button className="account-dashboard-toggle" aria-label="계정 정보 펼치기"><ChevronDown size={18} /></button></div>
...
function Footer({ go }: { go: (to: string) => void }) {
  ...
  return <footer className="site-footer"><div className="footer-inner"><div className="footer-brand"><strong>TISTORY</strong><p>티스토리는 Daum에서 <ProgressiveImage src={assetUrl('tistory/heart.png')} alt="사랑" /> 을 담아 만듭니다.</p><small>© Daum Corp.</small></div><div className="footer-links">{groups.map(([title, links]) => <FooterGroup key={title as string} title={title as string} links={links as string[][]} go={go} />)}</div></div></footer>
}
      ...
      <div className="home-frame">
        ...
        <aside className="tistory-right">
          ...
          <section className="sidebar-module character-module">
            ...
            <div className="character-grid">{popularCharacters.map(([name, query, image, color]) => <button key={name} style={{ backgroundColor: color }} onClick={() => go(`/market?q=${encodeURIComponent(`#${query}`)}`)}><span><ProgressiveImage src={image} alt="" /></span><strong>{name}</strong></button>)}</div>
          </section>
        </aside>
      </div>
    </main>
...
function BlogPage({ slug, go, user, onLogin }: { slug: string; go: (to: string) => void; user: User | null; onLogin: () => void }) {
  ...
  const toggleSubscription = async () => {
    ...
    try {
      ...
      queryClient.setQueryData<BlogData>(queryKey, { ...data, blog: { ...data.blog, isSubscribed: !subscribed, subscriberCount: Math.max(0, (data.blog.subscriberCount ?? 0) + (subscribed ? -1 : 1)) } })
      await queryClient.invalidateQueries({ queryKey: ['posts'] })
      void queryClient.invalidateQueries({ queryKey: ['home'] })
    } catch (e) { setMutationError((e as Error).message) } finally { setBusy(false) }
  }
  ...
  return <Shell go={go} user={user} onLogin={onLogin}><main id="main" className="creator-blog-page"><div className="creator-blog-shell">
    ...
    <section className="creator-shop"><header className="creator-section-head"><div><p className="creator-kicker">CURATOR'S SHOP</p><h2>{data?.blog.shopName || blankPlaceholder}</h2><span>{data?.blog.shopDescription || blankPlaceholder}</span></div>{mine ? <button onClick={() => go('/blog/me/manage/market')}>내 상품 관리 ↗</button> : <button onClick={() => go('/market')}>마켓 둘러보기 ↗</button>}</header>
...
function Editor({ id, go, user }: { id?: string; go: (to: string) => void; user: User }) {
  ...
  const loadTaxonomy = () => Promise.all([request<BlogCategory[]>('/blogs/me/categories'), request<BlogClassification[]>('/blogs/me/classifications')]).then(([nextCategories, nextClassifications]) => { setCategories(nextCategories); if (!id) setCategoryId(nextCategories.find((item) => item.isDefault)?.id ?? nextCategories[0]?.id ?? null); setClassifications(nextClassifications); setClassificationError('') }).catch((error) => setClassificationError(error.message))
...
function Manage({ path, go, user, onLogin, onUserChange }: { path: string; go: (to: string) => void; user: User | null; onLogin: () => void; onUserChange: (user: User) => void }) {
  ...
  return <Shell go={go} user={user} onLogin={onLogin}><main id="main" className="manage-console"><aside className="manage-sidebar"><p>MANAGEMENT</p><nav>{nav.map(([key, label, Icon]) => <button className={section === key ? 'active' : ''} key={key} onClick={() => go(key === 'overview' ? '/blog/me/manage' : `/blog/me/manage/${key}`)}><Icon size={17} />{label}</button>)}</nav></aside><section className="manage-workspace"><header className="manage-console-head"><div><p className="eyebrow">MY TISTORY</p><h1>{nav.find(([key]) => key === section)?.[1] ?? '블로그 관리'}</h1><span>{blog?.name ?? '블로그 정보를 불러오는 중입니다.'}</span></div><div><button onClick={() => go('/write')}><PenLine size={15} /> 새 글</button><button onClick={() => go('/market/new')}><Package size={15} /> 상품 등록</button><button onClick={() => blog && go(`/blog/${blog.slug}`)}>내 블로그 <ArrowRight size={15} /></button></div></header>{error && <p className="manage-error">{error}</p>}{section === 'overview' && <ManageOverview go={go} />}{section === 'settings' && blog && <ManageSettings blog={blog} setBlog={setBlog} user={user} onUserChange={onUserChange} />}{section === 'interests' && <ManageInterests user={user} onUserChange={onUserChange} />}{section === 'categories' && <ManageCategories go={go} />}{section === 'posts' && <ManagePosts go={go} />}{section === 'market' && <ManageMarket go={go} />}{section === 'trash' && <ManageTrash />}</section></main></Shell>
}
...
function ManageSettings({ blog, setBlog, user, onUserChange }: { blog: Blog; setBlog: (blog: Blog) => void; user: User; onUserChange: (user: User) => void }) {
  ...
  const [interests, setInterests] = useState<string[]>(user.interests ?? []); const [savedInterests, setSavedInterests] = useState<string[]>(user.interests ?? []); const [interestBusy, setInterestBusy] = useState(false)
  ...
  const saveInterests = async () => { if (!interests.length) return; setInterestBusy(true); setMessage(''); try { const result = await request<{ interests: string[] }>('/auth/interests', { method: 'PATCH', body: JSON.stringify({ interests }) }); setInterests(result.interests); setSavedInterests(result.interests); onUserChange({ ...user, interests: result.interests }); await Promise.all([queryClient.invalidateQueries({ queryKey: ['home'] }), queryClient.invalidateQueries({ queryKey: ['search'] })]); setMessage('관심분야를 저장했습니다.') } catch (e) { setMessage((e as Error).message) } finally { setInterestBusy(false) } }
  ...
  const upload = async (file: File, localPreview: string) => { setCropSource(''); setBusy(true); setPreview(localPreview); const form = new FormData(); form.append('file', file); try { const result = await request<{ profileImageUrl: string }>('/blogs/me/profile-image', { method: 'POST', body: form }); setPreview(result.profileImageUrl); setBlog({ ...blog, profileImageUrl: result.profileImageUrl }); window.dispatchEvent(new CustomEvent('blog-profile-updated', { detail: result.profileImageUrl })); await refreshBlogCaches(); setMessage('프로필 이미지를 저장했습니다.') } catch (e) { setPreview(blog.profileImageUrl ?? ''); setMessage((e as Error).message) } finally { setBusy(false) } }
  const remove = async () => { setBusy(true); try { await request('/blogs/me/profile-image', { method: 'DELETE' }); setPreview(''); setBlog({ ...blog, profileImageUrl: null }); window.dispatchEvent(new CustomEvent('blog-profile-updated', { detail: null })); await refreshBlogCaches(); setMessage('기본 이미지로 되돌렸습니다.') } catch (e) { setMessage((e as Error).message) } finally { setBusy(false) } }
...
function ManageInterests({ user, onUserChange }: { user: User; onUserChange: (user: User) => void }) {
  ...
  const save = async () => { if (!interests.length) return; setBusy(true); setMessage(''); try { const result = await request<{ interests: string[] }>('/auth/interests', { method: 'PATCH', body: JSON.stringify({ interests }) }); setInterests(result.interests); setSaved(result.interests); onUserChange({ ...user, interests: result.interests }); await Promise.all([queryClient.invalidateQueries({ queryKey: ['home'] }), queryClient.invalidateQueries({ queryKey: ['search'] })]); setMessage('관심분야를 저장했습니다.') } catch (error) { setMessage((error as Error).message) } finally { setBusy(false) } }
...
function ManageCategories({ go }: { go: (to: string) => void }) {
  ...
  const add = async (event: FormEvent) => { event.preventDefault(); if (!name.trim()) return; try { await request('/blogs/me/categories', { method: 'POST', body: JSON.stringify({ name }) }); setName(''); await load(); await invalidate() } catch (e) { setError((e as Error).message) } }
  const rename = async (item: BlogCategory) => { const next = prompt('새 카테고리 이름', item.name)?.trim(); if (!next || next === item.name) return; try { await request(`/blogs/me/categories/${item.id}`, { method: 'PATCH', body: JSON.stringify({ name: next }) }); await load(); await invalidate() } catch (e) { setError((e as Error).message) } }
  const remove = async (item: BlogCategory) => { if (!confirm(`‘${item.name}’ 카테고리를 삭제할까요?`)) return; try { await request(`/blogs/me/categories/${item.id}`, { method: 'DELETE' }); await load(); await invalidate() } catch (e) { setError((e as Error).message) } }
  const reorder = async (next: BlogCategory[]) => { const before = items; setItems(next); try { await request('/blogs/me/categories/order', { method: 'PATCH', body: JSON.stringify({ categoryIds: next.map((item) => item.id) }) }); await invalidate() } catch (e) { setItems(before); setError((e as Error).message) } }
...
function MarketRow({ item, go }: { item: MarketItem; go: (to: string) => void }) {
  return <article className="feed-row market-row">{item.thumbnailUrl && <button className="market-row-thumbnail has-image" style={{ backgroundImage: `url(${item.thumbnailUrl})` }} onClick={() => go(`/market/${item.id}`)} aria-label={`${item.title} 이미지`} />}<div><p className="post-blog">{item.category} · {conditionLabel[item.condition]}</p><h2><button onClick={() => go(`/market/${item.id}`)}>{item.title}</button></h2><p className="excerpt">{item.description}</p><p className="market-tags">{item.tags.map((tag) => <button key={tag} onClick={() => go(`/search?tab=market&q=${encodeURIComponent(`#${tag}`)}`)}>#{tag}</button>)}</p><small>{item.seller.nickname} · {item.status === 'SELLING' ? '판매 중' : item.status}</small></div><div className="row-stat"><strong>{item.pricePoints.toLocaleString()} P</strong></div></article>
}
...
function Market({ go, user, onLogin }: { go: (to: string) => void; user: User | null; onLogin: () => void }) {
  ...
  const marketQuery = useQuery({ queryKey: ['market', query, sort, user?.id ?? 'anonymous'], queryFn: () => request<MarketItem[]>(`/market/items?q=${encodeURIComponent(query)}&sort=${sort}&page=1&size=12`), refetchInterval: 30_000 })
...
function App() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      for (let attempt = 0; attempt < 3 && !cancelled; attempt++) {
        try {
          const data = await request<{ user: User; blog: Blog | null; requiresThirdPartyConsent: boolean }>('/me')
          if (cancelled) return
          let blog = data.blog
          if (blog) {
            try {
              const publicBlog = await request<{ blog: Blog }>(`/blogs/${blog.slug}?page=1&size=1`)
              blog = { ...blog, profileImageUrl: publicBlog.blog.profileImageUrl, subscriberCount: publicBlog.blog.subscriberCount }
            } catch { /* Keep authentication usable when public blog enrichment is temporarily unavailable. */ }
          }
          if (cancelled) return
  ...
  const publicWhileAuthLoads = path === '/' || path === '/login' || path === '/signup' || path === '/interests/mockup'
    ...
    || path === '/market/price-guide' || path === '/market/coupons'
