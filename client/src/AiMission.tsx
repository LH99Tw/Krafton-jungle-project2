...
function CharacterCard({ entry, index, onSelect }: { entry: AiCharacter; index: number; onSelect: () => void }) {
  ...
  return <button className="ai-character-card" style={{ '--ai-accent': palette.accent, '--ai-soft': palette.soft } as React.CSSProperties} onClick={onSelect}>
    <span className="ai-card-number font-jua">0{index + 1}</span><ProgressiveImage eager src={entry.image} alt={`${entry.name}, ${entry.role}`} /><span className="ai-card-shade" /><span className="ai-card-copy"><small className="font-jua">{entry.role}</small><strong className="font-jua">{entry.name}</strong><em>{entry.description}</em><b className="font-jua">이 캐릭터와 시작 <ArrowRight size={16} /></b></span>
  </button>
}
...
export function AiMissionPage({ controller, nickname, go }: { controller: Controller; nickname: string; go: (route: string) => void }) {
  ...
  if (choosing || !character) return <main id="main" className="ai-stage ai-casting"><div className="ai-casting-inner">
    <header><p>AI COMPANION · MISSION LAB</p><h1 className="font-maplestory">{nickname}님의<br />동료를 선택하세요.</h1><span>성격과 말투는 달라도 미션 진행도는 그대로 이어집니다.</span></header>
    ...
    <button className="ai-create-character" onClick={() => { setCreateNotice(true); window.setTimeout(() => setCreateNotice(false), 2600) }}><span className="ai-create-icon"><Plus size={20} /></span><span><strong className="font-jua">나만의 AI 캐릭터 생성하기</strong><small>외형과 말투를 직접 설정하는 기능이 곧 추가됩니다.</small></span><em className="font-jua">COMING SOON</em><ArrowRight size={18} /></button>
  ...
  return <main id="main" className="ai-stage ai-room" style={{ '--ai-accent': palette.accent, '--ai-soft': palette.soft } as React.CSSProperties}>
    <section className="ai-portrait-pane"><ProgressiveImage src={character.image} alt="" /><div className="ai-portrait-overlay" /><div className="ai-character-meta"><p>YOUR AI COMPANION</p><h1 className="font-jua">{character.name}</h1><span className="font-jua">{character.role}</span><div><i /> missions 활성화</div></div><button className="ai-switch font-jua" onClick={() => setChoosing(true)}><RotateCcw size={14} /> 캐릭터 변경</button></section>
    <section className="ai-chat-pane"><header><div><span className="ai-online" /><div><strong className="font-jua">{character.name}</strong><small>당신의 활동을 함께 보고 있어요</small></div></div><span className="font-jua">{completedCount}/3 COMPLETE</span></header><div className="ai-chat-log" ref={logRef}>{messages.map((entry) => <div className={`ai-bubble ${entry.sender}`} key={entry.id}>{entry.sender === 'ai' && <ProgressiveImage src={character.image} alt="" />}<p>{entry.body}</p></div>)}</div><form className="ai-chat-form" onSubmit={submit}><input value={input} onChange={(event) => setInput(event.target.value)} maxLength={300} placeholder={`${character.name}에게 무엇이든 말해보세요`} /><button aria-label="메시지 보내기" disabled={!input.trim()}><Send size={17} /></button></form><div className="ai-suggests"><button className="font-jua" onClick={() => { addChat('지금 미션 어떻게 해?'); addReply(replyFor('지금 미션 어떻게 해?', controller)) }}>미션 도움</button><button className="font-jua" onClick={() => { addChat('진행 상태 알려줘'); addReply(replyFor('진행 상태 알려줘', controller)) }}>진행 상황</button><button className="font-jua" onClick={() => { addChat('보상은 어떻게 받아?'); addReply(replyFor('보상은 어떻게 받아?', controller)) }}>보상 질문</button></div></section>
    <aside className="ai-mission-pane"><header><p>MISSION QUEUE</p><span className="font-jua">{completedCount} / {AI_MISSIONS.length}</span></header>{currentMission ? <div className="ai-current-mission"><small>NEXT MISSION · {currentMission.index}</small><h2 className="font-jua">{currentMission.title}</h2><p>{currentMission.description}</p><strong>+{currentMission.reward}P <em>지급 예정</em></strong><button className="font-jua" onClick={() => go(currentMission.route)}>{currentMission.action} <ArrowRight size={16} /></button></div> : <div className="ai-all-clear"><Sparkles size={28} /><h2 className="font-jua">오늘의 미션 완료</h2><p>모든 예정 보상이 기록됐어요.</p></div>}<div className="ai-mission-list">{AI_MISSIONS.map((mission) => { const done = Boolean(state.progress[mission.id].completedAt); return <div className={done ? 'done' : currentMission?.id === mission.id ? 'active' : ''} key={mission.id}><span className="font-jua">{done ? <Check size={14} /> : mission.index}</span><div><strong className="font-jua">{mission.title}</strong><small>+{mission.reward}P · 지급 예정</small></div></div> })}</div><footer>현재는 프론트엔드 미션 기록만 제공하며<br />실제 포인트 잔액은 변경하지 않습니다.</footer></aside>
  </main>
}
