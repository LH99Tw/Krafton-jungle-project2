...
export type AiMessage = { id: string; sender: 'ai' | 'user'; body: string; createdAt: string; emotion?: string; source?: 'model' | 'fallback' | 'template'; animate?: boolean }
...
export function useAiMission(userId: number | null, api: Requester) {
  ...
  useEffect(() => {
    const onActivity = (event: Event) => {
      ...
      setState((current) => {
        ...
        const characterName = AI_CHARACTERS.find((item) => item.id === current.characterId)?.name ?? 'AI 동료'
        const body = activity.milestone === 'title' ? `${characterName}: 제목을 정했구나! 이제 본문을 30자 이상 채워보자.` : activity.milestone === 'body' ? `${characterName}: 본문 조건도 채웠어! 임시저장하면 미션 완료야.` : `${characterName}: 관심분야를 연결했어! 이 상태로 글을 저장하면 완료돼.`
  ...
  const sendMessage = async (body: string) => { setSending(true); setError(''); const optimistic: AiMessage = { id: crypto.randomUUID(), sender: 'user', body, createdAt: new Date().toISOString() }; setState((current) => ({ ...current, messages: [...current.messages, optimistic] })); try { const result = await api<{ reply: string; emotion: string; source: 'model' | 'fallback'; userRemainingTurns: number; globalRemainingTurns: number; availability: AiServerState['availability']; dailyLimitResetAt: string }>('/ai/messages', { method: 'POST', body: JSON.stringify({ body, idempotencyKey: optimistic.id }) }); setState((current) => ({ ...current, userRemainingTurns: result.userRemainingTurns, globalRemainingTurns: result.globalRemainingTurns, availability: result.availability, dailyLimitResetAt: result.dailyLimitResetAt, messages: [...current.messages, { id: crypto.randomUUID(), sender: 'ai', body: result.reply, emotion: result.emotion, source: result.source, animate: true, createdAt: new Date().toISOString() }] })) } catch (cause) { setState((current) => ({ ...current, messages: current.messages.filter((message) => message.id !== optimistic.id) })); setError((cause as Error).message); await refresh() } finally { setSending(false) } }
  ...
  return { state, character, currentMission, completedCount, loading, sending, error, completion, dock, refresh, selectCharacter, startMission, pauseMission, clearHistory, sendMessage, setCompletion, setDock }
}
...
export function AiMissionPage({ controller, nickname, go }: { controller: Controller; nickname: string; go: (route: string) => void }) {
  ...
  if (choosing || !character) return <main id="main" className="ai-stage ai-casting"><div className="ai-casting-inner"><header><p>AI COMPANION · MISSION LAB</p><h1>{nickname}님의<br />동료를 선택하세요.</h1><span>대화는 외부 AI로 처리되며, 최근 대화와 민감정보를 제외한 취향 요약이 저장됩니다.</span></header><label className="ai-consent"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} /> AI 처리와 대화 저장 범위를 확인했으며 이에 동의합니다.</label><div className="ai-character-grid">{AI_CHARACTERS.map((entry, index) => <CharacterCard disabled={!consent || loading} entry={entry} index={index} onSelect={() => void selectCharacter(entry.id).then(() => setChoosing(false))} key={entry.id} />)}</div>{error && <p className="form-error">{error}</p>}</div></main>
...
export function AiCompanionDock({ controller, path, go }: { controller: Controller; path: string; go: (route: string) => void }) {
  const { character, currentMission, completion, dock, setCompletion, setDock } = controller
  const palette = useImagePalette(character?.image ?? '', character?.accent ?? '#ef9bb4')
