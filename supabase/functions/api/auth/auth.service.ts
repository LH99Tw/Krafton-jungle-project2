...
export const login = async (request: Request) => {
  ...
  return json({ data: { user: { id: user.id, email: user.email, nickname: user.nickname }, blog: blog ?? null, requiresThirdPartyConsent: !user.third_party_consent_decided_at, message: '로그인되었습니다.' } }, 200, { 'Set-Cookie': sessionCookie(newSessionId) })
}
...
export const me = async (request: Request) => {
  ...
  return json({ data: { user: { id: user.id, email: user.email, nickname: user.nickname, preferenceOnboardingCompleted: Boolean(user.preference_onboarding_completed_at), createdAt: user.created_at, updatedAt: user.updated_at }, requiresThirdPartyConsent: !user.third_party_consent_decided_at, blog: blog ?? null } })
}
